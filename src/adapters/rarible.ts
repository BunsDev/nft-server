...
import {
  MarketChainConfig,
  MarketConfig,
  Rarible as RaribleMarketConfig
} from "../markets";
import { RaribleProvider } from "../markets/RaribleProvider";
...
const LOGGER = getLogger("RARIBLE_ADAPTER", {
...
async function runSales(provider: AdapterProvider): Promise<void> {
  ...
  LOGGER.info("Fetching sales for Rarible collections:", collections.length);
...
async function run(provider: AdapterProvider): Promise<void> {
  ...
  try {
    ...
    await handleError(e, "Rarible-adapter");
  }
}

const RaribleAdapter: DataAdapter = { run };
...
if (!process.argv[2] && !process.env.RUN_CRON_NAME) {
  RaribleProvider.build(RaribleMarketConfig).forEach((p) =>
    spawnProviderChild(p)
  );
} else if (process.argv[2] === "provider-child") {
  const RaribleProviders = RaribleProvider.build(RaribleMarketConfig);
  const providerName = process.argv[3];
  const config = RaribleProviders.find(
    (p) => p.chainConfig.providerName === providerName
  );
  const provider = config.instantiate();
  if (cluster.isWorker && config.chainConfig.cluster) {
    ClusterWorker.create(process.env.WORKER_UUID, `RARIBLE`, provider);
  } else if (isPrimary()) {
    if (config.chainConfig.cluster) {
      ClusterManager.create(`RARIBLE`, provider);
    }
    RaribleAdapter.run(provider);
  }
}
