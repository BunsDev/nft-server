...
import blurproviders from "./blur";

const providers = blurproviders as Record<string, any>;
    ...
    for (const chain of chains) {
      ...
      for (const chainConfig of chainContracts) {
        ...
        if (process.env.ADAPTER_REPROCESS) {
          if (
            process.env.ADAPTER_REPROCESS !== "ALL" &&
            providerName !== process.env.ADAPTER_REPROCESS
          ) {
            continue;
          }
          chainConfig.deployBlock =
            parseInt(process.env.ADAPTER_REPROCESS_START_BLOCK) ||
            chainConfig.deployBlock;
