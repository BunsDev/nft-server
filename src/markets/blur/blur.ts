...
const LOGGER = getLogger("BLUR_PROVIDER", {
...
export default class BlurProvider
  extends BaseProvider
  implements IMarketOnChainProvider, IClusterProvider
{
  public CONTRACT_NAME = "blur";
  public market = Marketplace.Blur;
      ...
      for (
        ...
        try {
          ...
          this.MetricsReporter.submit(
            `blur.${chain}.contract_queryFilter.blockRange`,
            toBlock - fromBlock
          );
          this.MetricsReporter.submit(
            `blur.${chain}.contract_queryFilter.latency`,
          ...
          LOGGER.debug("Blur Events", { fromBlock, toBlock, events });
  ...
  public parseEvents(
    ...
    for (const event of events) {
      ...
      const { sell, buy } = parsed.decodedData;
      const { trader: sellTrader } = sell;

      const {
        trader: buyTrader,
        collection: buyCollection,
        tokenId: buyTokenId,
        paymentToken: buyPaymentToken,
        price: buyPrice,
        amount: buyAmount,
      } = buy;
      ...
      meta.push({
        buyer: buyTrader,
        contract: "blur",
        contractAddress: buyCollection,
        eventSignatures: [event.eventSignature],
        hash: event.transactionHash,
        logIndex: event.logIndex,
        payment: {
          address: buyPaymentToken,
          amount: buyPrice,
        },
        price: buyPrice,
        seller: sellTrader,
